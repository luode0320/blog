...
help() {
      echo "帮助: "
      echo ""
      echo "可用参数: install, invoke, query, info"
      echo ""
      echo "安装/升级链码 ./chaincode.sh install mychannel mycc 1.0 1"
      echo "查询链码 ./node.sh query mychannel mycc [\"get\",\"cat\"]"
      echo "调用链码 ./node.sh invoke mychannel mycc set [\"tom\\\",\"大懒猫\"]"
      echo "链码信息 ./node.sh info mychannel"
}
...
############################  提交链码 -> docker提交链码  #######################	
  ...
  if [ "${gojava}" = "golang" ]; then
    ...
    peer chaincode query -C $channelChainCode -n $nameChainCode -c '{\"Args\":[\"queryCat\",\"cat\"]}'
	"
}
