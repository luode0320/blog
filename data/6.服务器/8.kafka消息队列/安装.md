...
docker run -d \
  ...
  -e KAFKA_CFG_CONTROLLER_QUORUM_VOTERS=1@0.0.0.0:9093 \
  -e KAFKA_KRAFT_CLUSTER_ID=abcdefghijklmnopqrstuv \
  -e KAFKA_CFG_LOG_RETENTION_BYTES=53687091200 \
  -e KAFKA_CFG_LOG_SEGMENT_BYTES=1048576000 \
  -e KAFKA_LOG_DIRS=/bitnami/kafka/data \
  -v /usr/local/src/kafka/data:/bitnami/kafka/data \
  -p 9092:9092 \
  -p 9093:9093 \
...
# 消费者组的消费情况
docker exec kafka kafka-consumer-groups.sh --bootstrap-server localhost:9092 --group <group_name> --describe
...
docker exec kafka kafka-consumer-groups.sh \
  ...
  --reset-offsets \
  --to-earliest \
  --topic EXCHANGE_DATA_CCY_PAIR.SWFT:0 \
  --execute
...
docker exec kafka kafka-run-class.sh kafka.tools.GetOffsetShell \
  ...
  --topic EXCHANGE_DATA_CCY_PAIR.SWFT \
  --time -2 \
  --partitions 0
# 查询最新偏移量
docker exec kafka kafka-run-class.sh kafka.tools.GetOffsetShell \
