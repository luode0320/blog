...
例如：当按钮被点击时，`count++` 会执行，表示将 `count` 的值增加 1。
...
在上面的例子中，我们能够通过被触发事件的 `event.target` 访问到该 DOM 元素。

- `event.target` 是原生事件对象的一个属性，它指向触发事件的 DOM 元素。
- 也就是说，`event.target` 是一个引用，指向你点击的元素。
- `event.target.tagName` 返回的是触发事件的 DOM 元素的标签名（大写字母），例如：`BUTTON`、`DIV`、`P` 等。
...
> 请勿同时使用 `.passive` 和 `.prevent`，因为 `.passive` 已经向浏览器表明了你*不想*阻止事件的默认行为。
>
