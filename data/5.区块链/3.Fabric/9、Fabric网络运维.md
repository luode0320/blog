...
1. 获取系统通道的创世区块
    ...
    - 将新联盟TestConsortium配置定义`TestConsortium.json`添加到channel的`Consortiums`的`TestConsortium`
      中，并将其写入`sys_updated_config.json`
...
- 初始的时候 部署区块链在configtx.yaml 文件的Profiles: 策略配置中联盟 只配置了Org1 和Org2 当以全新组织Org3
  去创建通道（当然这里是指org1 和 org2 不加入新通道） 会报错：提示org3 不是联盟成员 没有权限创建 此篇文章教会怎么解决问题
