...
CREATE TABLE `error_code_translation` (
  ...
  `name` varchar(200) DEFAULT NULL COMMENT '项目名称(仅标识项目, 不用做查询条件)',
  `code` int NOT NULL COMMENT '错误码(唯一, 多个项目也不可重复, 用前缀数字区分项目)',
  ...
  UNIQUE KEY `idx_code` (`code`) USING BTREE COMMENT '错误码索引'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 ROW_FORMAT=DYNAMIC COMMENT='错误码翻译表(总覆盖比例达到全球人口约 85%)';


我现在需要新增一条数据, code=10000,zhcn="异常, 请稍后重试!"
