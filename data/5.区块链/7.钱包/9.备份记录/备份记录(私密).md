...
```sh
id=f713f1f6-bcf0-4bd8-aae0-7fd531f89232

Hello, Please help me check the order.If the exchange cannot continue, can you refund the user?
user address: 0x51376b2377c184e1bc2442F5329811E5F13bBD1c

https://etherscan.io/tx/0xd73fd289185af86e32e9362931e110cffbbafeed5e3192382553c538c2622eaf
```

```sh
id=b597ca1c-4de6-46b9-a509-77bb3cd56c9c

你好，请帮我查一下订单, 如果不能兑换请原地址退款。
用户原地址: rJ6GQUA2bcvTdxTsf9y1c2PrerXRuuTomQ

https://xrpscan.com/tx/BD4B450201D1FB6B88A10E2FA88EFC05DE224AF6A429B6BC3209DF0D8A4339F2

在我上面的那个请求中
"depositCoinFeeRate": "0.022" -> 渠道费率(ellipal的额外费用和swft交易费用)
"chainFee": "4" -> swft会扣除4个DOGE作为矿工费

渠道费率理解应该没问题
这里面4个DOGE作为矿工费是不管链上实际收取多少, 都是以4个DOGE作为矿工费预先扣除的是吗?
```
...
grep -r "转账 transaction or transactionMax =.*EPLBAC57D5903E24E1692CBC8EF70D6E045" \
...
grep ewtn5df6b8f4e5ea4f95918d76586af3dfbb /home/deployer/log/finance-go/logs/Transaction/Transaction.log
...
-- 查询 coinsAda 表
SELECT *
FROM ellpal.coinsAda
WHERE logo LIKE 'https://wallettest.ellipal.com/images/coins/%';

-- 确认无误后执行更新
UPDATE ellpal.coinsAda
SET logo = REPLACE(logo, '.svg', '.png')
WHERE logo LIKE 'https://wallettest.ellipal.com/images/coins/%.svg';

-- 查询 coinsbsc 表
SELECT *
FROM ellpal.coinsbsc
WHERE logo LIKE 'https://wallettest.ellipal.com/images/coins/%';

-- 确认无误后执行更新
UPDATE ellpal.coinsbsc
SET logo = REPLACE(logo, '.svg', '.png')
WHERE logo LIKE 'https://wallettest.ellipal.com/images/coins/%.svg';

-- 查询 coinsDefault 表
SELECT *
FROM ellpal.coinsDefault
WHERE logo LIKE 'https://wallettest.ellipal.com/images/coins/%';

-- 确认无误后执行更新
UPDATE ellpal.coinsDefault
SET logo = REPLACE(logo, '.svg', '.png')
WHERE logo LIKE 'https://wallettest.ellipal.com/images/coins/%.svg';

-- 查询 coinsEos 表
SELECT *
FROM ellpal.coinsEos
WHERE logo LIKE 'https://wallettest.ellipal.com/images/coins/%';

-- 确认无误后执行更新
UPDATE ellpal.coinsEos
SET logo = REPLACE(logo, '.svg', '.png')
WHERE logo LIKE 'https://wallettest.ellipal.com/images/coins/%.svg';

-- 查询 coinsflr 表
SELECT *
FROM ellpal.coinsflr
WHERE logo LIKE 'https://wallettest.ellipal.com/images/coins/%';

-- 确认无误后执行更新
UPDATE ellpal.coinsflr
SET logo = REPLACE(logo, '.svg', '.png')
WHERE logo LIKE 'https://wallettest.ellipal.com/images/coins/%.svg';

-- 查询 coinsftm 表
SELECT *
FROM ellpal.coinsftm
WHERE logo LIKE 'https://wallettest.ellipal.com/images/coins/%';

-- 确认无误后执行更新
UPDATE ellpal.coinsftm
SET logo = REPLACE(logo, '.svg', '.png')
WHERE logo LIKE 'https://wallettest.ellipal.com/images/coins/%.svg';

-- 查询 coinsheco 表
SELECT *
FROM ellpal.coinsheco
WHERE logo LIKE 'https://wallettest.ellipal.com/images/coins/%';

-- 确认无误后执行更新
UPDATE ellpal.coinsheco
SET logo = REPLACE(logo, '.svg', '.png')
WHERE logo LIKE 'https://wallettest.ellipal.com/images/coins/%.svg';

-- 查询 coinsmatic 表
SELECT *
FROM ellpal.coinsmatic
WHERE logo LIKE 'https://wallettest.ellipal.com/images/coins/%';

-- 确认无误后执行更新
UPDATE ellpal.coinsmatic
SET logo = REPLACE(logo, '.svg', '.png')
WHERE logo LIKE 'https://wallettest.ellipal.com/images/coins/%.svg';

-- 查询 coinsNew 表
SELECT *
FROM ellpal.coinsNew
WHERE logo LIKE 'https://wallettest.ellipal.com/images/coins/%';

-- 确认无误后执行更新
UPDATE ellpal.coinsNew
SET logo = REPLACE(logo, '.svg', '.png')
WHERE logo LIKE 'https://wallettest.ellipal.com/images/coins/%.svg';

-- 查询 coinsxdc 表
SELECT *
FROM ellpal.coinsxdc
WHERE logo LIKE 'https://wallettest.ellipal.com/images/coins/%';

-- 确认无误后执行更新
UPDATE ellpal.coinsxdc
SET logo = REPLACE(logo, '.svg', '.png')
WHERE logo LIKE 'https://wallettest.ellipal.com/images/coins/%.svg';
```
