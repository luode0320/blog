...
user address: UQDgYIUgIbxu-geES0wwokPTT9FHvuT7oXH8l74fHk50aJqU

https://tonscan.org/tx/c3565ee079b1e59616da98cedfe373bb635ed80fcb52b04e2aa50996600f31a7
```

```sh
id=e64574eb-b78b-4900-8099-e65704064530

你好，请帮我查一下订单, 如果不能兑换请原地址退款。
用户原地址: 1KnvdYUD8QceG2Sy6rXPeAJkTeaTFWxsR2


https://www.blockchain.com/explorer/transactions/bch/ac44e5de5c4ddde7dacbe71cb50765a87f97f186da7013354317b7406716fc1b
```
...
# 创建订单
tail -f -n 11111 /var/log/finance-go/logs/app.log | grep "CreateOrder"
...
tail -f -n 11111 /var/log/finance-go/logs/app.log | grep "当前币价priceMap总数量"
tail -f -n 11111 /var/log/finance-go/logs/app.log | grep "追踪超时订单状态" 
tail -f -n 11111 /var/log/finance-go/logs/app.log | grep "追踪超时订单状态异常" 
```
...
tail -f -n 111 /data/repos/exchange/logs/crypto-portal-5010.log | grep "创建买币卖币订单"
```
...
tail -f -n 111 /data/repos/exchange/logs/crypto-data-5001.log | grep "获取法币usd价格"
...
tail -f -n 111 /data/repos/exchange/logs/crypto-data-5001.log | grep -E "获取范围|获取汇率"
...
# 解压日志
gunzip /data/repos/exchange/logs/archive/exchange-ellipal-5075.log.2025-03-24.0.gz
tail -f -n 11111 /data/repos/exchange/logs/archive/exchange-ellipal-5075.log.2025-03-24.0 | grep "\-\-\-\-\-\-"
...
tail -f -n 11111 /data/repos/exchange/logs/exchange-ellipal-5075.log | grep "转账ws接收交易签名"
...
# 链上确认
tail -f -n 11111111 /data/repos/exchange/logs/exchange-ellipal-5075.log | grep "链上确认"
# 链上确认成功
...
tail -f -n 11111111 /data/repos/exchange/logs/archive/exchange-ellipal-5075.log.2025-03-24.0 | grep -E "对冲第一步|对冲初始化"
...
tail -f -n 11111111 /data/repos/exchange/logs/archive/exchange-ellipal-5075.log.2025-03-24.0 | grep --color=always -E "对冲初始化: 充币到交易所|充币到交易所, 状态:.*false|充币到交易所, 状态:|充币开始"

# 对冲提币
tail -f -n 11111111 /data/repos/exchange/logs/exchange-ellipal-5075.log | grep -E "对冲初始化: 提币到币池|提币到币池完成"
tail -f -n 11111111 /data/repos/exchange/logs/archive/exchange-ellipal-5075.log.2025-03-24.0 | grep -E "对冲初始化: 提币到币池|提币到币池完成"

# 对冲交易所到账, 卖现货平合约
tail -f -n 11111111 /data/repos/exchange/logs/exchange-ellipal-5075.log | grep -E "对冲初始化: 充币到交易所|交易所确认到账成功|对冲结束"
tail -f -n 11111111 /data/repos/exchange/logs/archive/exchange-ellipal-5075.log.2025-03-24.0 | grep -E "对冲初始化: 充币到交易所|交易所确认到账成功|对冲结束"

# 划转
tail -f -n 11111111 /data/repos/exchange/logs/archive/exchange-ellipal-5075.log.2025-03-24.0 | grep "划转"
...
# kafka消息监听器: 处理法币交易订单
tail -f -n 1111111 /data/repos/exchange/logs/crypto-strategy-5005.log | grep "kafka消息监听器"
tail -f -n 1111111 /data/repos/exchange/logs/crypto-strategy-5005.log | grep "kafka消息监听器: 处理法币交易订单"
tail -f -n 1111111 /data/repos/exchange/logs/crypto-strategy-5005.log | grep "kafka消息监听器: 处理法币交易订单, 更新到mysql数据库"
tail -f -n 1111111 /data/repos/exchange/logs/crypto-strategy-5005.log | grep "kafka消息监听器: 处理法币交易订单, orderId: bstn36f32439256c4795a927f1971b624fd3"

tail -f -n 1111111 /data/repos/exchange/logs/crypto-strategy-5005.log | grep "创建买币卖币订单"
```
...
# BTC
elp://1:1@sync2/EC02+_V4.3.0/3146B9B4BA527A436BBFA9F0C993B4E2B868CED5/兑换模拟数据/BTC[3GrV4Y1dF4xJxhSkSiyeG8kgE1vSp1NpeY[CF848C741D1989C0F779851B9ECBD6FDF9084D1382ED4DA19163185DFC4FE46F/17xNxkEwUDEPBUFT83z4RqpWRi5wL8zfuL
# ETH
