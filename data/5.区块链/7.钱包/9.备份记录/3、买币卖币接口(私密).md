...
```sh
curl --request GET \
     --url 'https://api.moonpay.com/v3/currencies/eth/buy_quote?baseCurrencyCode=usd&quoteCurrencyAmount=3&baseCurrencyAmount=200&extraFeePercentage=1&paymentMethod=paypal&areFeesIncluded=true&apiKey=pk_live_SGCdGCsO4JCl8YaMTMzCaYFLdiedTNS' \
     --header 'accept: application/json'
...
```sh
curl --request GET \
     --url 'https://api.moonpay.com/v3/currencies/eth/sell_quote?quoteCurrencyCode=usd&baseCurrencyAmount=3&apiKey=pk_live_SGCdGCsO4JCl8YaMTMzCaYFLdiedTNS' \
     --header 'accept: application/json'
```
...
```sh
curl --request GET \
     --url 'https://api.moonpay.com/v3/sell_transactions/ext/bc976ff6-c89d-4c35-9694-ae183758abf5?apiKey=pk_live_SGCdGCsO4JCl8YaMTMzCaYFLdiedTNS' \
     --header 'accept: application/json'
```
...
<head>
    ...
    <title>MoonPay JWT Request</title>
    <script src="https://cdn.jsdelivr.net/npm/@moonpay/auth-sdk"></script>
    <script>
        async function getPartnerJwtAndFetchTransactions() {
            try {
                // 初始化 SDK，传入你的公钥（在此例中使用了假设的公钥 'pk_test_123'）
                const sdk = new MoonPayAuthSDK('pk_live_SGCdGCsO4JCl8YaMTMzCaYFLdiedTNS');  // 替换为你的实际公钥

                // 获取 partnerJwt
                const partnerJwt = await sdk.partners.getPartnerToken();
                console.log('Partner JWT:', partnerJwt);
                ...
                console.log('MoonPay Transactions:', data);

                // 如果成功，显示数据
                alert('Transactions fetched successfully. Check the console.');
            } catch (error) {
...
<body>
    <h1>MoonPay API Example</h1>
    <button onclick="getPartnerJwtAndFetchTransactions()">Get Partner JWT & Fetch Transactions</button>
</body>
</html>
