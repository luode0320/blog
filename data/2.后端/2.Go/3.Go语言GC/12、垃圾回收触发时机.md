...
4. **完成清扫阶段 N+1**:
    - **循环清扫**: 使用 `for` 循环和 `sweepone` 函数来帮助完成清扫阶段 N+1。
    - **检查周期数**: 使用 `work.cycles.Load()` 来检查当前是否仍处于周期 N+1。
    - **调度**: 使用 `Gosched()` 来让出 CPU 时间片，以便其他 goroutine 可以运行。
5. **等待清扫完成**:
    - **循环等待**: 使用 `for` 循环和 `isSweepDone` 函数来等待清扫完成。
    - **调度**: 使用 `Gosched()` 来让出 CPU 时间片，以便其他 goroutine 可以运行。
6. **发布堆配置文件**:
    - **获取内存锁**: 使用 `acquirem()` 获取内存锁。
    - **检查周期数**: 检查当前是否仍处于周期 N+1 或者处于下一个周期的标记阶段。
    - **发布配置文件**: 如果条件满足，则调用 `mProf_PostSweep()` 来发布堆配置文件。
...
- **条件检查**:
    - **创建临时 gcTrigger 实例**: 创建一个临时的 `gcTrigger` 实例，其 `kind` 为 `gcTriggerTime`标识强制`GC`，`now`
      字段为当前时间。
    - **调用 `test` 方法**: 调用 `gcTrigger` 实例的 `test` 方法来检查是否需要触发 GC。
    - **检查是否处于空闲状态**: 检查 `forcegc.idle` 是否为 `true`，即 GC 是否处于空闲状态。
- **加锁**:
    - **加锁**: 使用 `lock` 函数加锁，以确保对 `forcegc` 结构体的操作是原子的。
- **设置 `forcegc.idle`**:
...
- **解锁**:
    - **解锁**: 使用 `unlock` 函数解锁，释放对 `forcegc` 结构体的独占访问。

这里有个test()方法内判断, 是否达到超时时间了。
...
func gcStart(trigger gcTrigger) {
	...
	setGCPhase(_GCmark) // 设置 GC 并发标记阶段
	gcBgMarkPrepare()   // 准备后台标记
	gcMarkRootPrepare() // 准备根标记
	gcMarkTinyAllocs()  // 标记所有活动的 tinyalloc 区块，它们需要像其他分配一样被标记为黑色。
...
1. **检查不可抢占或潜在不稳定的情况**:
    ...
    - **检查是否持有锁**: 检查是否持有锁 (`mp.locks > 1`) 或者是否不可抢占 (`mp.preemptoff != ""`)。
    - **释放内存锁**: 使用 `releasem(mp)` 释放内存锁。
    - **返回**: 如果处于不可抢占或潜在不稳定的情况，则直接返回。
2. **并发清扫剩余区间**:
    - **循环清扫**: 使用 `for` 循环和 `sweepone` 函数来帮助完成清扫阶段。
    - **检查过渡条件**: 使用 `trigger.test()` 检查过渡条件。
    - **增加清扫计数**: 使用 `sweep.nbgsweep++` 增加清扫计数。
3. **执行 GC 初始化和清扫终止过渡**:
    - **获取启动信号量**: 使用 `semacquire(&work.startSema)` 获取启动信号量。
    - **重新检查过渡条件**: 在过渡锁下重新检查过渡条件。
    - **升级模式**: 根据 `debug.gcstoptheworld` 的值来升级 GC 模式。
    - **获取信号量**: 使用 `semacquire(&gcsema)` 和 `semacquire(&worldsema)` 获取 GC 和世界信号量。
    - **设置用户强制**: 如果是用户强制触发，则设置 `work.userForced`。
    - **记录 GC 开始**: 如果启用了跟踪，则记录 GC 开始。
    - **检查 mcache**: 检查所有 Ps 的 mcache 是否已完成刷新。
    - **准备后台标记工作**: 使用 `gcBgMarkStartWorkers()` 准备后台标记工作。
    - **重置标记状态**: 使用 `gcResetMarkState()` 重置标记状态。
    - **设置 STW 进程数**: 设置 `work.stwprocs` 和 `work.maxprocs`。
    - **记录初始时间**: 使用 `nanotime()` 记录初始时间。
    - **停止世界**: 使用 `stopTheWorldWithSema(stwGCSweepTerm)` 停止世界。
    - **完成清扫**: 使用 `finishsweep_m()` 完成清扫。
    - **清除池**: 使用 `clearpools()` 清除池。
    - **增加周期数**: 使用 `work.cycles.Add(1)` 增加周期数。
    - **开始 GC 控制器周期**: 使用 `gcController.startCycle()` 开始 GC 控制器周期。
    - **通知 CPU 限制器**: 使用 `gcCPULimiter.startGCTransition()` 通知 CPU 限制器。
    - **禁用用户调度**: 如果是 STW 模式，则禁用用户 G 的调度。
4. **进入并发标记阶段**:
    ...
    - **启用写屏障**: 使用 `atomic.Store(&gcBlackenEnabled, 1)` 启用写屏障。
    - **获取内存锁**: 使用 `acquirem()` 获取内存锁。
    - **并发标记**: 使用 `systemstack()` 和 `startTheWorldWithSema()` 进行并发标记。
    - **释放世界信号量**: 使用 `semrelease(&worldsema)` 释放世界信号量。
    - **释放内存锁**: 使用 `releasem(mp)` 释放内存锁。
    - **调度**: 如果是 STW 模式，则使用 `Gosched()` 调度。
5. **释放启动信号量**:
    - **释放信号量**: 使用 `semrelease(&work.startSema)` 释放启动信号量。
...
1. **计算堆目标和最小触发阈值**:
    - `goal, minTrigger := c.heapGoalInternal()`：获取堆目标和最小触发阈值。
2. **处理特殊情况**:
    - 如果`c.heapMarked >= goal`，则：
        - 返回 `goal, goal`，表示垃圾回收应在目标大小时触发。
3. **设置触发阈值的下限**:
    - 如果`minTrigger < c.heapMarked`，则：
        - 设置 `minTrigger = c.heapMarked`，确保触发阈值不低于 heapMarked。
4. **计算触发阈值的下限**:
    - 计算`triggerLowerBound`，确保触发阈值不会过低。
        - 如果`minTrigger < triggerLowerBound`，则：
            - 设置 `minTrigger = triggerLowerBound`。
5. **计算最大触发阈值**:
    - 对于较小的堆，将最大触发阈值设置为目标大小与存活堆之间的 maxTriggerRatio。
    - 对于较大的堆，将最大触发阈值设置为目标大小减去最小堆大小。
        - 如果`maxTrigger < minTrigger`，则：
            - 设置 `maxTrigger = minTrigger`。
6. **计算触发阈值**:
