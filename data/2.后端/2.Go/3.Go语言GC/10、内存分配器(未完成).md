...
- Go 语言使用一个全局的堆内存区域来存放所有动态分配的对象。
    - 内存分配器负责在堆上分配和回收内存。

2. **多级分配策略**:

    - Go 使用多级分配策略来提高内存分配的效率。

    - 分配器分为几个层次：page 页分配器、span 块分配器和object 对象分配器。

        1. **page 页分配器**:

            - 页分配器负责从操作系统获取物理页，并将这些页组织成 span。
            - 页通常是 4KB 的大小。

        2. **span 块分配器**:

            - span 是一组连续的页，具有相同的大小。
            - span 的大小范围从 8KB 到 1GB。
            - span 分配器负责管理 span 的分配和回收。

        3. **object 对象分配器**:

            - 对象分配器负责在 span 上分配对象。
            - 对象分配器使用一个简单的分配算法，如首次适应算法。

3. **gc 垃圾回收器**:

    - Go 语言使用一种称为“三色标记-清扫”(tracing GC)的垃圾回收算法。

    - 垃圾回收器周期性地运行，回收不再使用的内存。

4. **并发性和安全性**:

    - Go 的内存分配器是并发安全的，可以在多个 goroutine 之间共享。
    - 内存分配器的设计考虑到了并发性和锁的竞争最小化。
...
- Span class 是 span 的种类，但 span class 的大小与 span 的大小并不是直接正比关系。
...
这里看似有4个定义, 实际上我们只需要懂申请内存的时候, 是想通过计算此次需要的内存大小 size, 从而获取一个适合内存大小的
span
...
刚刚特别提到了`scav`时小对象垃圾回收的区域, 那么就有大对象垃圾回收的区域。
...
- 如果`emtpy`里也没有符合条件的span，mcentral 会向 mheap 申请span。
