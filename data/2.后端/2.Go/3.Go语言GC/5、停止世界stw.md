...
1. **Trace 开始**:
   - 如果启用了追踪，记录 STW 的开始。
2. **获取调度器锁**:
   - 获取 `sched.lock` 锁，确保在 STW 期间调度器状态的一致性。
3. **设置停止计数器**:
   - 设置 `stopwait` 计数器为 `gomaxprocs`，表示需要停止的 P 的数量。
   - 设置 `gcwaiting` 标志，表示正在进行垃圾回收。
4. **抢占所有 P**:
   - 调用 `preemptall` 强制所有 P 预抢占，确保它们都停止。
5. **停止当前 P**:
   - 将当前 P 的状态设为 `_Pgcstop`，表示它已停止。
6. **停止系统调用中的 P**:
   - 遍历所有 P，如果一个 P 的状态为 `_Psyscall`（表示在系统调用中），则将其状态改为 `_Pgcstop`。
   - 更新 `stopwait` 计数器。
7. **停止空闲的 P**:
