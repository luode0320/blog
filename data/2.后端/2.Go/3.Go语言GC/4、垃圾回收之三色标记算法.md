...
1. **函数签名**:
   - `markroot` 函数接收三个参数：
     ...
     - `flushBgCredit` 是一个布尔值，指示是否应该将产生的 GC 工作量刷新到后台信用池。
2. **禁用抢占**:
   - 函数注释中指出此函数必须在禁用抢占的环境下运行，这是因为函数内部使用了 `gcWork`，它可能需要不受中断地完成工作。
3. **处理不同的根对象类型**:
...
4. **扫描 goroutine 堆栈**:
   - 当 `i`  的值落在 goroutine 堆栈的范围内时，函数会扫描相应的 goroutine 堆栈。
     - 如果 `i` 不在有效范围内，函数会抛出错误。
     - 获取当前要扫描的 goroutine (`gp`)。
     - 如果 goroutine 处于等待或系统调用状态，记录等待时间。
     - 如果是自我扫描（即扫描当前 goroutine），将状态更改为 `_Gwaiting` 以避免死锁。
     - 使用 `suspendG` 函数暂停 goroutine，以便扫描其堆栈。
       - 如果 goroutine 已经死亡，则标记为扫描完成并返回。
       - 如果 goroutine 已经被扫描过，则抛出异常。
       - 使用 `scanstack` 函数来扫描 goroutine 的堆栈。
...
5. **更新工作量计数器**:
   - 根据扫描的根对象类型，函数可能会更新不同的工作量计数器。
   - 如果 `flushBgCredit` 为 `true`，则将产生的工作量刷新到后台信用池。



###  扫描 goroutine 的堆栈，将堆栈上的所有指针标记为灰色
...
1. **函数签名**:
   - `scanConservative`函数接收五个参数：
     ...
     - `state` 是一个指向 `stackScanState` 结构的指针，表示堆栈扫描的状态信息。
2. **调试输出**:
   - 如果启用了`debugScanConservative`，则打印保守扫描的区间，并使用`hexdumpWords`打印区间的内容，其中：
     - `$` 表示未被 ptrmask 标记的字。
     - `@` 表示指向堆栈的指针。
     - `*` 表示指向已分配对象的指针。
3. **遍历区间**:
...
5. **检查指针**:
   - 获取当前字的值 `val`。
   - 检查`val`是否指向堆栈内。
     - 如果是，则将 `val` 放入 `state` 的指针队列，并标记为保守指针。
   - 检查`val`是否指向一个堆 span。
     - 如果不是，则跳过。
   - 检查`val`是否指向一个已分配的对象。
     - 如果不是，则跳过。
   - 如果 `val` 指向一个已分配的对象，则调用 `greyobject` 函数来标记该对象。
6. **标记对象**:
...
1. **函数签名**:
   - `greyobject`函数接收六个参数：
     ...
     - `objIndex` 是一个 `uintptr` 类型的整数，表示对象在 span 中的索引。
2. **检查指针对齐**:
   - 检查 `obj` 是否是指针对齐的。如果不是，则抛出错误。
3. **获取标记位**:
   - 获取对象的标记位 `mbits`。
4. **检查标记**:
   - 如果启用了 `useCheckmark`，则使用 `setCheckmark` 函数来检查和设置标记位。
   - 如果未启用`useCheckmark`，则进行以下操作：
     - 如果 `debug.gccheckmark > 0` 并且对象是自由的，则抛出错误。
     - 如果对象已经被标记，则返回。
     - 否则，设置对象的标记位。
     - 标记 span。
     - 如果对象不需要扫描，则将其标记为黑色并返回。
5. **预取对象**:
   - 使用 `sys.Prefetch` 预取对象，以便后续的处理更快。
6. **排队扫描**:
...
1. **功能概述**：
   - `gcDrain` 函数是在并发标记阶段执行的，主要目的是执行扫描工作，即黑化灰色对象，直到无法获得更多的工作。
   - 根据传入的标志参数，函数可以在不同条件下返回，比如在 Goroutine 被抢占时返回、有其他工作要做时返回等。
2. **初始化**：
   - 获取当前正在运行的 Goroutine（`gp := getg().m.curg`）。
   - 设置一些标志和参数，比如是否可抢占、是否需要刷新工作信用等。
3. **根对象标记任务处理**：
   - 处理未完成的根对象标记任务，即扫描根对象并将其标记为灰色。
   - 根据条件进行自我抢占检查，如果满足条件则提前结束任务处理。
4. **堆对象处理**：
   - 循环处理堆中的对象，不断尝试获取对象并扫描（黑化灰色对象）。
   - 在处理过程中，根据工作量和条件进行工作负载平衡、信用冲刷等操作。
   - 检查是否需要将本地扫描工作信用冲刷到全局账户，以便其他协程协助处理。
5. **结束操作**：
   - 最后，对剩余的扫描工作信用进行冲刷操作，确保工作的完整性和准确性。



### 遍历将下一个子对象标记为灰色,完成之后将本对象标记为黑色
...
4. **处理大对象**:
   - 如果对象大小超过`maxObletBytes`，则将其拆分成 oblets。
     ...
     - 计算 oblet 的大小，确保不超过 `maxObletBytes`。
5. **获取指针掩码**:
   - 使用 `heapBitsForAddr` 获取指针掩码 `hbits`。
6. **扫描对象**:
   - 循环遍历对象中的指针。
     - 使用 `hbits.nextFast` 和 `hbits.next` 获取下一个可能的指针地址 `addr`。
     - 更新 `scanSize` 以跟踪最远的指针位置。
...
### 写屏障缓冲区刷新到垃圾回收的工作队列, 并解决漏标-悬挂指针问题
...
1. **函数签名**:
   - `wbBufFlush1` 函数接收一个 `*p` 类型的指针，表示当前处理器 P。
2. **获取缓冲区中的指针**:
...
3. **毒化缓冲区**:
   - `pp.wbBuf.next = 0`：将缓冲区的 next 指针设置为 0，防止在处理缓冲区期间有新的指针被加入。
4. **Checkmark 模式**:
   - 如果使用 Checkmark 模式，则遍历所有指针并调用`shade`函数将它们标记为灰色。
...
1. **确保只有一个线程运行 ragged barrier**:

   - `semacquire(&work.markDoneSema)`：确保只有一个线程在同一时间运行 ragged barrier。
...
4. **刷新本地缓冲区并收集 flushedWork 标志**:

   - `gcMarkDoneFlushed = 0`：初始化 flushedWork 标志计数器。
   ...
   - `systemstack(func() { ... })`
     ...
     - `casGToWaiting(gp, _Grunning, waitReasonGCMarkTermination)`：标记用户栈为可以抢占，以便它可以被扫描。

     - `forEachP(func(pp *p) { ... })`

       ：遍历所有 P，执行以下操作：

       - `wbBufFlush1(pp)`：刷新写屏障缓冲区。
       - `pp.gcw.dispose()`：刷新 gcWork。
       - `if pp.gcw.flushedWork { ... }`：收集 flushedWork 标志。

     - `casgstatus(gp, _Gwaiting, _Grunning)`：恢复 goroutine 状态。
...
6. **执行标记终止阶段**:
   ...
   - `gcComputeStartingStackSize()`：计算初始栈大小。

   - `atomic.Store(&gcBlackenEnabled, 0)`：禁用标记。

   - `gcCPULimiter.startGCTransition(false, now)`：通知 CPU 限制器。

   - `gcWakeAllAssists()`：唤醒所有被阻塞的协助。

   - `semrelease(&work.markDoneSema)`：释放转换锁。

   - `schedEnableUser(true)`：重新启用用户 goroutines。

   - `gcController.endCycle(now, int(gomaxprocs), work.userForced)`：结束周期。
