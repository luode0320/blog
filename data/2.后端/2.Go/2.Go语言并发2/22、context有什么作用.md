...
1. **取消操作**：`context` 提供了一种优雅的方式来取消长时间运行的函数调用。这在处理网络请求、数据库操作或其他 I/O 操作时特别有用，可以防止资源泄漏。
2. **超时控制**：你可以为 `context` 设置一个超时时间，如果在指定时间内操作未完成，`context` 将会自动取消，从而终止相关操作。
3. **传递请求范围的值**：`context` 可以携带任意类型的数据，这在处理 HTTP 请求时非常有用，可以在请求的生命周期内传递用户身份、跟踪 ID 或其他元数据。
