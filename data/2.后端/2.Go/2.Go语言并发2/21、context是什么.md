...
1. **`context.Context`**: 这是一个接口，定义了所有 `context` 类型必须实现的方法。这些方法包括 `Done()`，`Err()`，`Value(key interface{}) interface{}` 等。
...
4. **`context.WithCancel(parent Context)`**: 返回一个子 Context 和一个 Cancel 回调函数，可以调用 Cancel 回调函数来取消子 Context。
5. **`context.WithDeadline(parent Context, deadline time.Time)`**: 返回一个子 Context，该 Context 将在指定的截止时间后自动取消。
6. **`context.WithTimeout(parent Context, timeout time.Duration)`**: 类似于 `WithDeadline`，但在给定的时间间隔后取消 Context。
